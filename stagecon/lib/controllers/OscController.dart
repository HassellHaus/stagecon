...
class OSCcontroler extends GetxController {
  ...
  callLogEventListener(OSCMessage message) {
    for (var element in logListeners) {
      element(message);
    }
  }
  ...
  parse(OSCMessage msg) async {
    ...
    try {
      ...
      else if ((stopwatchCommand || countdownCommand || timerCommand) && msg.address.contains("/deleteAll", 15)) {
        // offset of 16 because the timer is shorter than the other two commands
...
// Safely parse arguments
          ...
          /// Match arguments based on length and types
          if (argLen > 0) title = _extractString(msg.arguments[0]) ?? '';

          if (argLen == 2) {
            // Pattern: String Int?
            if (msg.arguments[1] is int) {
              ttl = msg.arguments[1] as int;
            } else {
              id = _extractString(msg.arguments[1]);
            }
          } else if (argLen == 3) {
            // Pattern: String Int? String?
            if (msg.arguments[1] is int) {
              ttl = msg.arguments[1] as int;
              id = _extractString(msg.arguments[2]);
            } else {
              content = _extractString(msg.arguments[1]);
              ttl = _extractInt(msg.arguments[2]) ?? 3000;
            }
          } else if (argLen == 4) {
            // Pattern: String String? Int? String?
            content = _extractString(msg.arguments[1]);
            ttl = _extractInt(msg.arguments[2]) ?? 3000;
            id = _extractString(msg.arguments[3]);
          }
        ...
        }  else if (msg.address.contains("/revokeAll", 17)) {
          // deletes every message
          ScMessage.deleteAll();
        } else if (msg.address.contains("/revoke", 17)) {
          // deletes a message
          if (argLen == 0) {
            Get.showSnackbar(GetSnackBar(
              title: "OSC Error: ${msg.address}",
              duration: const Duration(seconds: 3),
              message: "An id is required to delete a message",
            ));
          }
          String id = msg.arguments[0] as String;
      ...
      //MARK: Cuelights
      if (msg.address.startsWith("/stagecon/cuelight")) {
        // offset: 18

        //update cuelight state
        if (msg.address.contains("/state", 18)) {
          /// "cuelight id" "state"

          if (argLen < 2) {
            throw Exception(
                "Cuelight must have an id and a state.  /stagecon/cuelight/state \"SOME ID\" [inactive|standby|active]");
          }
          String id = msg.arguments[0] as String;
  ...
  String? _extractString(dynamic arg) => arg is String ? arg : null;
  int? _extractInt(dynamic arg) => arg is int ? arg : null;
}
